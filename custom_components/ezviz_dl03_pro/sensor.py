...
class EzvizBatterySensor(CoordinatorEntity, SensorEntity):
    def __init__(self, coordinator, serial):
        ...
        self._attr_device_info = DeviceInfo(identifiers={(DOMAIN, serial)}, name="Zamek DL03 Pro")

    @property
    def native_value(self):
        # Poprawiona ścieżka na podstawie Twojego JSONa
        device_data = self.coordinator.data.get(self.serial, {})
        opts = device_data.get("STATUS", {}).get("optionals", {})
        power = opts.get("multiPower", [])
